...
class LightDecoupledHead(nn.Module):
    def __init__(self, ch=256, nc=80, anchors=()):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
        self.cls_convs = Conv(ch, 64, 3, 1, 1)
        self.reg_convs = Conv(ch, 64, 3, 1, 1)
        self.obj_convs = Conv(ch, 64, 3, 1, 1)
...
class CBAM(nn.Module):
    ...
    def __init__(self, c):  # ch_in, ch_out, kernel, stride, padding, groups
...
class SPD3(nn.Module):
    # Changing the dimension of the Tensor
    def __init__(self, c1, c2):
        super().__init__()
        self.conv = Conv(9*c1, c2)
...
class pool_and_conv(nn.Module):
    def __init__(self, c, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        self.cv = Conv(c, c, 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)

    def forward(self, x):
        return torch.cat([self.m(x), self.m(x)], 1)
...
class SPPC(nn.Module):
    ...
    def __init__(self, c1, c2):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_ * 15, c2, 1, 1)
        self.mc1 = pool_and_conv(c_)
        self.mc2 = pool_and_conv(2*c_)
        self.mc3 = pool_and_conv(4*c_)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y1 = self.mc1(x)
            y2 = self.mc2(y1)
            return self.cv2(torch.cat([x, y1, y2, self.mc3(y2)], 1))
