...
from utils.general import coco80_to_coco91_class, check_dataset, check_img_size, check_requirements, \
    ...
    increment_path, colorstr, print_args, non_max_suppression_rotate
...
def run(data,
    ...
    for batch_i, (img, targets, segments, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        t3 = time_sync()
        if cls_theta == 0:
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls)  # return out: x,y,x,y,conf,cls
        else: # rotate detection
            out = non_max_suppression_rotate(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls, cls_theta=cls_theta, rotate_nms=rotate_nms)  # return out: x,y,x,y, conf,cls,theta
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                correct = process_batch(predn, labelsn, iouv)   # 300*10
                if plots:
        ...
        if plots and batch_i < 3:
            ...
            if cls_theta > 1:
                ...
                targets = torch.cat((targets[:, :6], theta / cls_theta), 1)
            Thread(target=plot_images, args=(img, targets, cls_theta, paths, f, names), daemon=True).start()
            f = save_dir / f'val_batch{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(img, output_to_target(out, cls_theta), cls_theta, paths, f, names), daemon=True).start()
