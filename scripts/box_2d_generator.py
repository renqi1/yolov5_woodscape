...
--label_tyoe: generate yolo format label: 'yolo' for horizontal box detecton,
            'yolo_rotate' for rotating box, others generate default label
...
from parsers.detection.helpers import read_image_as_rgb, \
    generate_2d_boxes, generate_2d_boxes_yolo, generate_2d_boxes_yolo_rotate, write_to_file
...
def parser_arguments():
    ...
    arg_parser.add_argument("--src_path", type=str, default="WoodScape_ICCV19/instance_annotations",
                            help="instance annotations directory")
    arg_parser.add_argument("--dst_path", type=str, default="WoodScape_ICCV19/yolo_rotate",
    ...
    arg_parser.add_argument("--rgb_image_path", type=str, nargs="?",  default="WoodScape_ICCV19/rgb_images",
                            help="rgb_image directory")
    arg_parser.add_argument("--label_type", type=str, default="yolo_rotate", help='none, yolo, yolo_rotate')
    ...
    if os.path.isdir(args.src_path) and \
        ...
        return args.src_path, args.dst_path, box_2d_classes_mapping,\
            instance_classes_mapping, args.rgb_image_path, args.label_type
    else:
...
if __name__ == "__main__":
    ...
    # parser arguments
    src_path, dst_path, box_2d_info, instance_classes_mapping, rgb_image_path, label_type = parser_arguments()
    ...
    for source_file_name in tqdm(trace_list):
        ...
        parser = AnnotationDetectionParser(json_ann_name,
                                           (shape[1], shape[0]),
                                           filter_ann_params)
        if label_type == 'yolo_rotate':
            img_poly_annotations, stats = generate_2d_boxes_yolo_rotate(shape, parser, class_names, class_colors, class_ids, class_obj_thresh,
                                                        image_rgb=image_debug, debug=debug)
        elif label_type == 'yolo':
            img_poly_annotations, stats = generate_2d_boxes_yolo(shape, parser, class_names, class_colors, class_ids, class_obj_thresh,
                                                        image_rgb=image_debug, debug=debug)
        else:
            img_poly_annotations, stats = generate_2d_boxes(parser, class_names, class_colors, class_ids, class_obj_thresh,
                                                            image_rgb=image_debug, debug=debug)
        write_to_file(dst_file_name, img_poly_annotations, yolo=True)
