...
class Annotator:
    ...
    def rotate_label(self, xywh, theta, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
            if isinstance(xywh, torch.Tensor):
                xywh = xywh.cpu().numpy()
            elif isinstance(xywh, list):
                xywh = np.array(xywh)
            if isinstance(theta, torch.Tensor):
                theta = theta.cpu().numpy()
            if isinstance(self.im, Image.Image):
                self.im = np.array(self.im)
            center, w, h = xywh[:2], xywh[2], xywh[3]
            # longedge --> opencv
            if theta > 0.5:     # θ ∈ (0,1) which indicates (0, 180)
                theta -= 0.5
                w, h = h, w
            theta = theta * 3.1416
            ...
            cv2.drawContours(self.im, [point_list], -1, color, self.lw)

            if label:
                tf = max(self.lw - 1, 1)  # font thicknes
                x_label, y_label = int(center[0]), int(center[1])
                w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 3, thickness=tf)[0]  # text width, height
                cv2.rectangle(
                                self.im,
                                (x_label, y_label),
...
def output_to_target(output, cls_theta):
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf]
    targets = []
    if cls_theta != 0:
        for i, o in enumerate(output):
            for *box, conf, cls, theta in o.cpu().numpy():
                targets.append([i, cls, *list(*xyxy2xywh(np.array(box)[None])), conf, theta])
    else:
        for i, o in enumerate(output):
...
def plot_images(images, targets, cls_theta=0, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            ...
            para = 6 if cls_theta == 0 else 7
            labels = ti.shape[1] == para  # labels if no conf column
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    if cls_theta == 0:
                        ...
                        annotator.rotate_label(new_box, ti[j, 6], label, color=color)

    annotator.im = annotator.im if isinstance(annotator.im, Image.Image) else Image.fromarray(annotator.im)
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    x = pd.DataFrame(b.transpose(), columns=['x', 'y', 'width', 'height'] if b.shape[0] == 4 else ['x', 'y', 'width', 'height', 'angle'])
    # seaborn correlogram
...
# def feature_visualization(x, module_type, stage, n=9, save_dir=Path('runs/detect/exp')):
#     """
...
#             fig, ax = plt.subplots(3, 3, tight_layout=True)  # 8 rows x n/8 cols
#             ax = ax.ravel()
