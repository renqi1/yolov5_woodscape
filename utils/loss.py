...
from utils.metrics import bbox_iou, rotate_iou
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.hyp['cls_theta'] > 1 and self.riou:
                    _, pbox_theta = torch.max(ps[:, -self.hyp['cls_theta']:], 1, keepdim=True)
                    rpbox = torch.cat((pxy, pwh, pbox_theta/self.hyp['cls_theta']), 1)
                    _, tbox_theta = torch.max(ttheta[i], 1)
                    iou = rotate_iou(rpbox.T, tbox[i], tbox_theta/self.hyp['cls_theta'])
                else:
                ...
                if self.hyp['cls_theta'] > 1:
                    t_theta = ttheta[i].type(ps.dtype)  # target theta_gaussian_labels
                    ltheta += self.BCEtheta(ps[:, class_index:], t_theta)
                elif self.hyp['cls_theta'] == 1:
                    t_theta = ttheta[i].type(ps.dtype)
                    ltheta += self.MSEtheta(ps[:, class_index].sigmoid().squeeze(-1), t_theta.squeeze(-1))
    ...
    def build_targets(self, p, targets):
        ...
        na, nt, np = self.na, targets.shape[0], targets.shape[1]  # number of anchors, targets, number of parameters
        tcls, tbox, indices, anch,  ttheta = [], [], [], [], []
        gain = torch.ones(np+1, device=targets.device)  # normalized to gridspace gain
