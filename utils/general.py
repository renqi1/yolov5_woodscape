...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)   # box(4)+cls_conf(1)+cls(1) !!!
...
def non_max_suppression_rotate(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300, cls_theta=1, rotate_nms=False):
    """Runs Non-Maximum Suppression (NMS) on inference results_obb
    Args:
        prediction (tensor): (b, n_all_anchors, [x y w h conf cls theta])
        agnostic (bool): True = NMS will be applied between elements of different categories
        labels : () or

    Returns:
        list of detections, len=batch_size, on (n,7) tensor per image [x,y,x,y,conf,cls,theta] theta ∈ [0,1])
    """

    nc = prediction.shape[2] - 5 - cls_theta  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    class_index = nc + 5
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if cls_theta == 1:  # regression task
            theta_pred = x[:, -1].sigmoid().unsqueeze(1)  # θ ∈ (0,1)
        else:   # classification task
            _, theta_pred = torch.max(x[:, class_index:], 1, keepdim=True)  # [n_conf_thres, 1] θ ∈ int[0, 179]
            theta_pred = theta_pred/cls_theta  # [n_conf_thres, 1] θ ∈ [0, 1)

        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 5:class_index] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), theta_pred[i]), 1)   # box(4)+cls_conf(1)+cls(1)+theta(1) !!!
        else:  # best class only
            conf, j = x[:, 5:class_index].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), theta_pred), 1)[conf.view(-1) > conf_thres]
        ...
        if rotate_nms:  # nms_rotate takes too much time
            rboxes, scores = torch.cat((x[:, :4] + c, x[:, -1].unsqueeze(1)), 1), x[:, 4]  # rboxes (offset by class), scores
            i = nms_rotate(rboxes, scores, iou_thres)  # NMS
        else:
            boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
            i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        if i.shape[0] > max_det:  # limit detections
...
def nms_rotate(boxes, scores, iou_thres):
    ...
    while sort_boxes.size(0):
        ...
        sort_boxes = sort_boxes[1:][rotate_ious < iou_thres]
...
def rotate_iou(box1, box2):
    r_b1 = get_rotated_coors(box1.squeeze(0))
    ious = []
...
def get_rotated_coors(rbox):
    if isinstance(rbox, torch.Tensor):
        rbox = rbox.cpu().numpy()
    elif isinstance(rbox, list):
        rbox = np.array(rbox)
    center, w, h, theta = rbox[:2], rbox[2], rbox[3], rbox[4]
    if theta > 0.5:
        theta -= 0.5
        w, h = h, w
    theta = theta * 3.1416
