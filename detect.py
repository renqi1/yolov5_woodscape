...
from utils.general import apply_classifier, check_img_size, check_imshow, check_requirements, check_suffix, colorstr, \
    increment_path, non_max_suppression, non_max_suppression_rotate, print_args, save_one_box, scale_coords, set_logging, \
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in cl.unique():
                    n = (cl == c).sum()  # detections per class
                ...
                if cls_theta == 0:
                    ...
                    for *xyxy, conf, cls, theta in reversed(det):
                        xywh_img = xyxy2xywh(torch.tensor(xyxy).view(1, -1))
                        if save_txt:  # Write to file
                            xywh = (xywh_img / gn).view(-1).tolist()
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            c = int(cls)  # integer class
                            label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                            annotator.rotate_label(xywh_img.view(-1), theta, label, color=colors(c, True))
